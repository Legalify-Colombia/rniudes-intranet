...
import { Eye, Users, FileText, Plus, Clock, CheckCircle, Trash2 } from "lucide-react";
...
export function InternationalizationManagers() {
  ...
  const [managerWorkPlans, setManagerWorkPlans] = useState<any[]>([]);
  ...
  const openManagerDetails = async (manager: any) => {
    ...
    try {
      ...
      // Load assigned work plans for this manager
      const plansResult = await fetchWorkPlansForManager(manager.id);
      if (plansResult.data) {
        setManagerWorkPlans(plansResult.data);
      }
    } catch (error) {
      console.error("Error loading manager data for details:", error);
    }
  ...
  const handleAssignPlan = async () => {
    ...
    try {
      ...
      if (result.data) {
        ...
        openManagerDetails(selectedManager); // Reload details to show new plan
      }
    } catch (error) {
