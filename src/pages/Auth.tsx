...
export default function Auth() {
  ...
  const positions = getValidPositions().filter(pos => pos && typeof pos === 'string' && pos.trim().length > 0 && validatePosition(pos));
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <div className="flex justify-center mb-4">
            <img src="https://udes.edu.co/images/logo/logo-con-acreditada-color.png" alt="UDES Logo" className="h-16 w-auto" />
          </div>
          <CardTitle className="text-2xl font-bold text-primary">ORBIS UDES 1.0</CardTitle>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {isSignUp && <>
                ...
                <div>
                  ...
                  <Input id="documentNumber" type="text" value={formData.documentNumber} onChange={e => setFormData(prev => ({
                ...prev,
                ...
                <div>
                  ...
                  <Select value={formData.position || undefined} onValueChange={handlePositionChange}>
                    ...
                    <SelectContent>
                      {positions.filter(position => validatePosition(position)).map(position => <SelectItem key={position} value={position}>
                            {position}
                          </SelectItem>)}
                    </SelectContent>
                  </Select>
                ...
                {formData.position === 'Gestor de Internacionalización' && <>
                    <div>
                      ...
                      <Input id="weeklyHours" type="number" value={formData.weeklyHours} onChange={e => setFormData(prev => ({
                  ...prev,
                  weeklyHours: parseInt(e.target.value) || 0
                    ...
                    <div>
                      ...
                      <Input id="numberOfWeeks" type="number" value={formData.numberOfWeeks} onChange={e => setFormData(prev => ({
                  ...prev,
                  numberOfWeeks: parseInt(e.target.value) || 16
                }))} required />
                    </div>
                  </>}
              </>}
