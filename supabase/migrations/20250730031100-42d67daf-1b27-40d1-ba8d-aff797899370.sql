...
DROP POLICY IF EXISTS "Solo directores de programa pueden aprobar planes" ON custom_plans;
DROP POLICY IF EXISTS "Coordinadores pueden agregar observaciones" ON custom_plans;

-- Nueva política: Solo directores de programa pueden aprobar planes (cambiar status)
CREATE POLICY "Solo directores de programa pueden aprobar planes"
ON custom_plans
FOR UPDATE
USING (
  -- El gestor puede editar su propio plan en draft/rejected
  (manager_id = auth.uid() AND status IN ('draft', 'rejected')) OR 
  -- Los directores de programa pueden aprobar/rechazar planes enviados
