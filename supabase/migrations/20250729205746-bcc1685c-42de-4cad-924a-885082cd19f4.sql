-- Corregir las funciones para que no dependan de plan_fields que no existe
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(cp.title, '') as objectives,
    COALESCE((
      SELECT jsonb_agg(
        jsonb_build_object(
...
-- Actualizar la función get_pending_work_plans_with_details
