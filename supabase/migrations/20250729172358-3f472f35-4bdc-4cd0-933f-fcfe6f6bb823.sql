-- Primero eliminar las funciones existentes para poder recrearlas
DROP FUNCTION IF EXISTS public.get_complete_work_plan_details(uuid);
DROP FUNCTION IF EXISTS public.get_pending_work_plans_with_details();

-- Fase 1: Reparar la función get_complete_work_plan_details para incluir asignaciones de horas
...
-- Actualizar la función get_pending_work_plans_with_details para incluir datos de asignaciones
...
BEGIN
  ...
  GROUP BY 
    cp.id, cp.title, cp.manager_id, cp.plan_type_id, cp.status, 
    cp.created_at, cp.updated_at, cp.submitted_date, cp.approved_date,
    cp.approval_by, cp.approval_comments, p.full_name, p.email, 
